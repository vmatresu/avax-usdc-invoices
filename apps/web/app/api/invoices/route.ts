...
export async function GET(request: NextRequest) {
  ...
  if (!invoiceManagerAddress) {
    return NextResponse.json({ error: 'Invoice manager contract address not configured' }, { status: 500 })
  }

  try {
    const logs = await getMerchantInvoiceCreatedLogs(merchant as `0x${string}`, invoiceManagerAddress)

    const invoices = await Promise.all(
      logs.map(async (log) => {
        const invoice = await getInvoice(log.args.invoiceId as `0x${string}`, invoiceManagerAddress)
        return {
